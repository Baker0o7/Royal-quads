...
export default function Profile() {
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-5">
      <SectionCard>
        ...
        <form onSubmit={handleAuth} className="flex flex-col gap-3">
          {!isLogin && <input type="text" placeholder="Full Name" value={name} onChange={e => setName(e.target.value)} className={inputCls} required />}
          <input type="tel" placeholder="Phone Number" value={phone} onChange={e => setPhone(e.target.value)} className={inputCls} required />
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 p-4 rounded-2xl font-bold text-lg hover:bg-stone-800 dark:hover:bg-stone-200 transition-colors mt-2 disabled:opacity-50">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-5">
      <div className="bg-emerald-600 dark:bg-emerald-700 text-white p-6 rounded-3xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3">
        <SectionCard className="text-center py-4">
          ...
          <div className="text-xs text-stone-500 dark:text-stone-400 mt-1 flex items-center justify-center gap-1">
      ...
      <section>
        <h2 className="text-sm font-bold mb-3 flex items-center gap-2">
          <History className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Ride History
        </h2>
        {history.length === 0
          ? <div className="text-center p-8 bg-white dark:bg-stone-900 rounded-3xl border border-stone-200 dark:border-stone-800 text-stone-500 text-sm">No rides yet. Time to hit the dunes! 🏜️</div>
          : (
            <div className="flex flex-col gap-3">
              {history.map(b => (
                <div key={b.id} className="bg-white dark:bg-stone-900 p-4 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="font-bold text-stone-900 dark:text-stone-100">{b.quadName}</h3>
                      <div className="flex items-center gap-1 text-xs text-stone-400 mt-0.5">
                        <Calendar className="w-3 h-3" />{new Date(b.startTime).toLocaleDateString()}
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-emerald-600 dark:text-emerald-400">{b.price.toLocaleString()} KES</div>
                      {b.promoCode && <div className="text-xs text-stone-400 line-through">{b.originalPrice.toLocaleString()} KES</div>}
                    </div>
                  </div>
                  <div className="flex items-center gap-4 pt-3 border-t border-stone-100 dark:border-stone-800">
                    ...
                    {b.rating != null && (
                      <div className="flex items-center gap-0.5 ml-auto text-amber-400">
                        ...
                        <span className="text-xs font-bold text-stone-500 dark:text-stone-400">{b.rating}/5</span>
                      </div>
                    )}
                  </div>
                </div>
