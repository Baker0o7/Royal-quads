    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <div className={cn(
        ...
        <div className="relative z-10 p-8 flex flex-col items-center">
          <p className="font-mono text-[10px] tracking-[0.2em] text-white/40 uppercase mb-5">
            {booking.quadName}
          </p>
          ...
          <div className="relative w-36 h-36 mb-5">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
              ...
              <circle cx="60" cy="60" r="54" fill="none"
                stroke={isOvertime ? '#f87171' : '#c9972a'}
                strokeWidth="6" strokeLinecap="round"
                strokeDasharray={circumference}
                strokeDashoffset={circumference * (1 - (isOvertime ? 1 : progress / 100))}
          ...
          <div className="w-full bg-white/5 rounded-2xl p-4 flex flex-col gap-3 text-sm border border-white/5">
            {[
              ...
              <div key={l} className="flex justify-between">
                <span className="text-white/40 font-mono text-xs">{l}</span>
            ...
            {booking.quadImei && (
              <div className="flex justify-between pt-2 border-t border-white/5">
                ...
                <button onClick={() => navigate(`/track/${booking.quadImei}`)}
                  className="font-medium text-sm text-[#e8b84b] hover:text-[#c9972a] transition-colors flex items-center gap-1">
      ...
      <AnimatePresence>
        {isOvertime && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="w-full max-w-sm bg-red-50 dark:bg-red-900/15 text-red-700 dark:text-red-400 p-4 rounded-2xl flex items-start gap-3 border border-red-200/60 dark:border-red-800/40">
            <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold text-sm">Please return the quad</p>
              <p className="text-xs mt-0.5 opacity-80">Your rental period has ended.</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
