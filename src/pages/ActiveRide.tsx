...
export default function ActiveRide() {
  ...
  useEffect(() => {
    api.getActiveBookings().then(data => {
      ...
      if (current) { setBooking(current); setSecondsLeft(calcSeconds(current)); }
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try { await api.completeBooking(Number(id)); navigate(`/receipt/${id}`); }
  ...
  const progressPct = Math.min(100, Math.max(0, ((totalSecs - secondsLeft) / totalSecs) * 100));
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div className={cn(
        ...
        isOvertime ? 'bg-red-600 dark:bg-red-700' : 'bg-stone-900 dark:bg-stone-800', 'text-white'
      )}>
        <div className="relative z-10 flex flex-col items-center">
          ...
          {!isOvertime && (
            <div className="w-full bg-white/10 rounded-full h-2 mb-6 overflow-hidden">
              <div className="bg-emerald-400 h-full rounded-full transition-all duration-1000" style={{ width: `${progressPct}%` }} />
            </div>
          )}
          <div className="w-full bg-white/10 rounded-2xl p-4 flex flex-col gap-2.5 text-left backdrop-blur-sm">
            {[['Rider', booking.customerName], ['Duration', `${booking.duration} min`], ['Price', `${booking.price.toLocaleString()} KES`]].map(([l, v]) => (
      ...
      {isOvertime && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-4 rounded-2xl flex items-start gap-3 border border-red-200 dark:border-red-800 w-full max-w-sm">
          <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
          <div>
            <p className="font-bold text-sm">Please return the quad</p>
            <p className="text-sm mt-1">Your rental has ended. Return to the starting point.</p>
          </div>
        </motion.div>
      )}

      <button onClick={handleComplete} disabled={completing}
        className="w-full max-w-sm bg-emerald-600 text-white p-4 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 hover:bg-emerald-700 transition-colors shadow-lg disabled:opacity-60 active:scale-[0.99]">
        {completing
