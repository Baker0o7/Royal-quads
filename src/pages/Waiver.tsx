...
export default function Waiver() {
  ...
  useEffect(() => {
    api.getActiveBookings().then(data => {
      ...
      if (b) setBooking(b);
      else navigate(`/ride/${bookingId}`, { replace: true });
    });
  }, [bookingId, navigate]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-5">
      ...
      <div className="p-5 rounded-2xl t-card max-h-60 overflow-y-auto text-xs leading-relaxed space-y-3"
        ...
        <p className="font-display font-bold text-sm" style={{ color: 'var(--t-text)' }}>INDEMNITY & WAIVER AGREEMENT</p>
        <p>I, <strong style={{ color: 'var(--t-text)' }}>{booking.customerName}</strong>, acknowledge that quad biking carries inherent risks including physical injury or death. I voluntarily choose to participate in activities provided by <strong style={{ color: 'var(--t-text)' }}>Royal Quads Mambrui</strong>.</p>
        ...
        <p>I hereby <strong style={{ color: 'var(--t-text)' }}>release, indemnify, and hold harmless</strong> Royal Quads Mambrui, its owners, employees and agents from any claims, damages, or liability arising from my participation.</p>
        <p className="font-mono text-[10px]">By signing below I confirm I have read and agree to all terms of this waiver.</p>
      </div>
      ...
      <div>
        ...
        <div className="rounded-xl overflow-hidden border-2 border-dashed"
          style={{ borderColor: 'var(--t-border)', background: 'white' }}>
        ...
        {!hasStroke && (
          <p className="text-[10px] font-mono text-center mt-1" style={{ color: 'var(--t-muted)' }}>
      ...
      <button onClick={handleSign} disabled={!agreed || !hasStroke || signing} className="btn-primary">
        {signing ? <><Spinner /> Processing…</> : 'Sign & Start Ride'}
      </button>

      <button onClick={() => navigate(`/ride/${bookingId}`)}
        className="text-xs text-center font-mono transition-opacity hover:opacity-70"
        style={{ color: 'var(--t-muted)' }}>
        Skip for now
      </button>
    </motion.div>
