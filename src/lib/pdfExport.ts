  ...
  try {
    container.innerHTML = `
      <div style="background:#1a1612; color:white; padding:28px 24px; text-align:center;">
        ...
        <div style="font-size:12px; color:#c9972a; margin-top:10px; font-family:monospace; letter-spacing:2px;">#${esc(booking.receiptId)}</div>
      </div>
      ...
      <div style="background:#f5f0e8; padding:0 20px;">
        ${rows.map(([label, value]) => `
          <div style="display:flex; justify-content:space-between; align-items:center;
            ...
            <span style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:#7a6e60; font-family:monospace;">${esc(label)}</span>
            <span style="font-size:13px; font-weight:500; color:#1a1612; text-align:right; max-width:55%;">${esc(value)}</span>
          </div>
        `).join('')}
      </div>
      ...
      ${(booking.overtimeCharge || 0) > 0 ? `
        <div style="background:rgba(239,68,68,0.08); margin:0 20px; border-radius:10px; padding:10px 14px;
          ...
          <span style="font-size:11px; color:#ef4444; font-family:monospace;">
            Overtime ${booking.overtimeMinutes}min x ${OVERTIME_RATE} KES
          </span>
      ...
      <div style="text-align:center; padding:20px; border-top:1px dashed rgba(201,185,154,0.3); margin:0 20px;">
        <div style="font-size:12px; color:#7a6e60; font-style:italic; margin-bottom:6px;">
          Thank you for riding with {BUSINESS_NAME}!
        </div>
    ...
    console.error('[pdfExport] PDF generation failed:', e);
    throw e;
  } finally {
